...
if __name__=='__main__':
    ...
    if DETECTED:
        ...
        STAGEDIR   = '/scratch/generateDRM_%s' %(OBJECT)
        #  STAGING THE FILES...
        ...
        stageSet.stageOut('%(OUTPUT_DIR)s/gll_cspec_bn%(TRIGGER_NAME)s_v%(version)02d.rsp'% locals()) # DRM
        stageSet.stageOut('%(OUTPUT_DIR)s/gll_DRM_bn%(TRIGGER_NAME)s_v%(version)02d.root'% locals()) # DRM
        stageSet.stageOut('%(OUTPUT_DIR)s/gll_edisp_bn%(TRIGGER_NAME)s_v%(version)02d.png'% locals()) # DRM
        stageSet.stageOut('%(OUTPUT_DIR)s/gll_effarea_bn%(TRIGGER_NAME)s_v%(version)02d.png'% locals()) # DRM
        stageSet.stageOut('%(OUTPUT_DIR)s/gll_mcvar_bn%(TRIGGER_NAME)s_v%(version)02d.png'% locals()) # DRM
        
        cmd='python -m mkdrm_ez -i %(LLEIFILE)s -ft2 %(FT2_s)s -outdir %(STAGEDIR)s/ -v %(VERSION)d ' %locals()
        cmd+='-mcbase %(MCBASEDIR)s ' % locals()
        cmd+='-ra %(RA).4f -dec %(DEC).4f -n %(OBJECT)s -t %(TRIGTIME).4f -d %(DURATION).1f -off %(OFFSET)s -dt %(DT)s -b %(BEFORE)s -a %(AFTER)s -r %(RADIUS)s -zenith %(ZENITHMAX)s -theta %(THETAMAX)d' % locals()    
