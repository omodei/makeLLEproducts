...
if __name__=='__main__':
    ...
    if DETECTED:
        MCToGenerateFile = '%(OUTPUT_DIR)s/MCConfig_bn%(TRIGGER_NAME)s_v%(version)02d.txt'% locals()
        GRBNAME=OBJECT
        
        print '--------------------------------------------------: Generation of the TEXT file to be used in the generation of LLE Montecarlo DATA'
        txt = "# Configuration file for %s\n" % GRBNAME
        txt+="NAME                      = '%s'\n" % GRBNAME
        txt+="DURATION                  = %.2f\n" % (DETECT_TLAST-DETECT_TFIRST)
        txt+="MET                       = %.4f\n" % (TRIGTIME+DETECT_TFIRST)
        txt+="RA                        = %.4f\n" % RA
        txt+="DEC                       = %.4f\n" % DEC
        ...
        print txt
        print '...generating the MC...'
        cmd='/nfs/farm/g/glast/u26/MC-tasks/GRBSimulator-Pass7/config/GenerateJobs.py %(MCToGenerateFile)s 1000 -p' % locals()
